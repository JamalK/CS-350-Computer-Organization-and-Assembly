...
//
 ...
 // Analysis of bit strings
  struct Commands{
    unsigned short opCode;      // -op- 0000 0000 0000
    unsigned short intSecond;   // 0000 scnd 0000 0000
    unsigned short NZP ;        // 000N ZP00 0000 0000
    unsigned short offset;      // 0000 - 9dt offset -
    unsigned short trap;        // 0000 0000 trap trap
    unsigned short last;        // 0000 0000 0000 last
...
void dump_memory(CPU *cpu, Address from, Address to, int nonzero_only, int counter)
{
    ...
    for(i = cpu->memory[j]; i <= end; i++)
    {
        ...
        {
            printf("x%04X: x%04hX %10d\n", i, cpu->memory[count], cpu->memory[count]);
            count++;
    ...
    switch ((Commands.opCode)) // ** Note ** place "op(cpu->instr_reg))"
    {
        case  0:
            if (Commands.last < 0){
                ...
                printf("BR   %d, CC = Z, go to x%04X+6 = x%04X\n", Commands.last, order, order + Commands.last);
...
void instr_ADD(CPU *cpu)
{
    printf("ADD  R%d, R%d, 1;  R2 <- x300E+1 = x300F; CC = P\n", Commands.NZP, Commands.NZP); // x3007: x14A1
}
...
void instr_AND(CPU *cpu)
{
    ...
    printf("AND  R%d, R%d, 0;  R%d <- x0000 & x0000 = x0; CC = Z\n", Commands.NZP, Commands.NZP, Commands.NZP); //x7FFA: x5B60
}
...
void instr_LDR(CPU *cpu)
{
	...
	int dst = (u_reg/512) % 8;
	int src = (u_reg/64) % 8;
	printf("\nR%04hX <- R%04hX + x%04hX\n", dst, src, immed);
	cpu->reg[dst] = cpu->reg[src] + immed;
}

// -------------------- LEA --------------------
//
void instr_LEA(CPU *cpu, order)    // Done
{
...
void instr_ST(CPU *cpu)
{
     ...
     */
    cpu->memory[cpu->pgm_counter + Commands.offset] = cpu->reg[Commands.NZP];
    
    printf("ST R%d, -32;  M[PC-32] = M[x7FD2] <- x7FFF\n", Commands.NZP   ); // x7FF1: x31E0
}
...
void instr_STR(CPU *cpu)
{
    ...
    // printf("STR  R2, R1, -32;  M[x8000-32] = M[x7FE0] <- x7FFF\n"); //x7FF5: x7460
...
void instr_TRAP(CPU *cpu)
{
    ...
    {
        printf("TRAP x22 (PUTS): hello, world!\n");
        /* IDEA
